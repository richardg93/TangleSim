...
Tangle::Tangle() try : m_genesisBlock(new Tx) {
     ...
     tipSelectSeed = std::chrono::system_clock::now().time_since_epoch().count();
...
//This enables you to instantiate Txs from within TxActor as they require a shared_ptr on construction to record the issuer

std::shared_ptr<TxActor> TxActor::getptr() {
...
void TxActor::attach(std::vector<t_ptrTx>& storedTips, omnetpp::simtime_t attachTime, t_txApproved& chosen, bool kind ) {
     ...
     try {
         ...
         for (int i = 0; i < APPROVE_VAL; i++) {
             ...
             if(chosen[i].get() != nullptr) { // if only one tip available (at start) will only be one actual tx approved - throws seg fault if not accounted for
                 chosen[i]->m_approvedBy.push_back(m_MyTx.back());

                 if(!chosen[i]->isApproved) {
                     chosen[i]->firstApprovedTime = attachTime;//with firstApprovedTime and timeAttached as field - we can compute the age of a transaction
                 }
...
int TxActor::_computeWeight(std::vector<t_ptrTx>& visited, t_ptrTx current, omnetpp::simtime_t timeStamp) {
    ...
    for(int i = 0; i < current->m_approvedBy.size(); i++) {
        //std::vector<t_ptrTx>::iterator it = std::find(visited.begin(), visited.end(), current->m_approvedBy[i]);
...
t_ptrTx TxActor::WalkTipSelection(t_ptrTx start, double alphaVal, std::vector<t_ptrTx>& tips, omnetpp::simtime_t timeStamp){
    ...
    while( !isRelativeTip(current, tips) ) {
        ...
        if(currentView.size() == 1) {
        ...
        } else { //if more than one find the max weight
            ...
            } else { // if at least one non heaviest still available pick between them

                std::uniform_real_distribution<double> walkChoice(0.0, 1.0);

                if(walkChoice(getTanglePtr()->getRandGen()) < alphaVal) {
                    current = heaviestTx;
                } else { //otherwise choose a random site
                    if(currentView.size() == 1) { //if only one left after removing heaviest use the remaining one
                        current = currentView[0];

                    } else { //otherwise pick at random
